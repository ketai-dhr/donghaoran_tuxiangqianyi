...
for epoch in range(settings.EPOCHS):
    ...
    with tqdm(total=settings.STEPS_PER_EPOCH, desc='Epoch {}/{}'.format(epoch + 1, settings.EPOCHS)) as pbar:
        ...
        for step in range(settings.STEPS_PER_EPOCH):
            ...
            pbar.update(1)
            progressbarOne['value'] += 100 / (settings.EPOCHS * settings.STEPS_PER_EPOCH)
            root.update()
        # 每个epoch保存一次图片
        utils.save_image(noise_image, 'images/train_output.jpg')

quit_ = tkinter.Button(root, text="完成退出", command=quit) 
